...
class CDEKService {
  ...
  async getOrderByUuid(uuid: string) {
    const token = await this.getProdToken();
    const response = await axios.get(`https://api.cdek.ru/v2/orders/${uuid}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  }
  ...
  async getCDEKDeliveryPeriods(address: string, fromAddress?: string, tariffCode?: number, options?: { postalCode?: string, city?: string, region?: string, code?: string, fias_guid?: string, from_code?: string, from_fias_guid?: string, from_postal_code?: string }): Promise<{
    periods: Array<{
      period_min: number;
      period_max: number;
      description: string;
    ...
    try {
      ...
      // Если явно переданы параметры отправления — используем их
      if (options.from_code) fromCityCode = options.from_code;
      if (options.from_fias_guid) fromFiasGuid = options.from_fias_guid;
      if (options.from_postal_code) fromPostalCode = options.from_postal_code;
