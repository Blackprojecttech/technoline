...
router.post('/', auth, admin, async (req, res) => {
  try {
    ...
    // Создаем долг только если это не услуга и есть поставщик
    if (arrival.supplierId && arrival.supplierName && arrival.items.some(item => !item.isService)) {
      const debt = new Debt({
...
router.delete('/:id', auth, admin, async (req, res) => {
  try {
    ...
    if (receiptsWithThisArrival.length > 0) {
      ...
      const errorMessage = `Нельзя удалить приход, так как следующие товары из него уже проданы:\n\n${conflictDetails.join('\n')}\n\nСначала удалите указанные чеки или обратитесь к администратору.`;
