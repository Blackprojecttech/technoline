...
const deliveryMethodSchema = new mongoose.Schema({
  ...
  deliveryTimeFrom: { type: String, match: /^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, default: null },
  deliveryTimeTo: { type: String, match: /^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, default: null },
  deliveryDayOffset: { type: Number, min: 0, default: 0 }
}, {
  timestamps: true
});
...
async function addDeliveryMethods() {
  try {
    ...
    createdMethods.forEach((method, index) => {
      ...
      let timeInfo = '';
      if (method.orderTimeFrom && method.orderTimeTo && method.deliveryTimeFrom && method.deliveryTimeTo) {
        const dayOffset = method.deliveryDayOffset || 0;
        const dayText = dayOffset === 0 ? 'сегодня' : dayOffset === 1 ? 'завтра' : `через ${dayOffset} дня`;
        timeInfo = ` | Заказ: ${method.orderTimeFrom}-${method.orderTimeTo}, Доставка: ${method.deliveryTimeFrom}-${method.deliveryTimeTo} (${dayText})`;
      }
