...
const Orders: React.FC = React.memo(() => {
  ...
  const { data, isLoading, error, refetch } = useQuery<OrdersResponse>({
    queryKey: ['adminOrders', currentPage, selectedStatus, deliveryFilter, pageSize],
    queryFn: () => fetchOrders(currentPage, selectedStatus, pageSize, deliveryFilter),
    retry: 1,
    retryDelay: 1000,
    enabled: !!localStorage.getItem('admin_token'),
  ...
  const handleDeliveryFilterChange = useCallback((filter: string) => {
    ...
    setCurrentPage(1);
    // Если пользователь явно выбрал "Все", сбрасываем флаг первой загрузки
    if (filter === '') {
      setIsFirstLoad(false);
    }
  }, []);
  ...
  const handleClearFilters = useCallback(() => {
    ...
    setDeliveryFilter('');
    setCurrentPage(1);
    // Не сбрасываем isFirstLoad, чтобы не сработала автоматическая установка фильтра
  }, []);
  ...
  const columns = React.useMemo(() => [
    ...
    },
    {
      ...
      key: 'customer',
      render: (_: any, record: Order) => (
        <CustomerCell userId={record.userId} />
      )
    },
    {
      title: 'Артикул',
  ...
  const createArrivalFromOrder = (formValues: any, items: any[]) => {
    try {
      // Получаем существующие приходы
      const existingArrivals = JSON.parse(localStorage.getItem('arrivals') || '[]');
      ...
      items.forEach(item => {
        ...
        const arrivalItem = {
          id: `arrival_${Date.now()}_${Math.random()}`,
          productId: item.id.startsWith('arrival_item_') ? `order_product_${Date.now()}_${Math.random()}` : item.id,
      ...
      itemsBySupplier.forEach((supplierItems, supplierId) => {
        ...
        const newArrival = {
          id: `order_arrival_${Date.now()}_${Math.random()}`,
          date: new Date().toISOString(),
          supplierId: supplierId,
          supplierName: supplierName,
          notes: formValues.notes || '',
          items: supplierItems,
          totalQuantity: supplierItems.reduce((sum: number, item: any) => sum + item.quantity, 0),
          totalValue: supplierItems.reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0)
        };
        
        newArrivals.push(newArrival);
      });

      // Сохраняем новые приходы
      const updatedArrivals = [...newArrivals, ...existingArrivals];
      localStorage.setItem('arrivals', JSON.stringify(updatedArrivals));

      // Также сохраняем в старом формате для совместимости
      const oldFormatArrivals: any[] = [];
      newArrivals.forEach(arrival => {
        arrival.items.forEach((item: any) => {
          ...
          oldFormatArrivals.push(oldFormatItem);
        });
      });

      // Добавляем к существующему старому формату
      const existingOldFormat = JSON.parse(localStorage.getItem('arrivals_old_format') || '[]');
      const updatedOldFormat = [...oldFormatArrivals, ...existingOldFormat];
      localStorage.setItem('arrivals_old_format', JSON.stringify(updatedOldFormat));

      // Показываем сообщение об успехе
      if (newArrivals.length === 1) {
        message.success(`Создан приход: ${newArrivals[0].totalQuantity} шт. на сумму ${newArrivals[0].totalValue.toLocaleString('ru-RU')} ₽`);
      } else {
        message.success(`Создано ${newArrivals.length} приходов для разных поставщиков`);
      }

      // Обновляем страницу, если мы на странице приходов
    ...
    <>
      ...
      <div className="orders-header-bar" style={{
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', justifyContent: 'flex-start', flex: 2 }}>
          ...
          <Button onClick={handleClearFilters} size="middle" type="default" style={{ borderRadius: 8, boxShadow: '0 2px 8px rgba(239,68,68,0.08)' }}>Сбросить фильтры</Button>
        </div>
