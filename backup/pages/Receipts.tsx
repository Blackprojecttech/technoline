...
const Receipts: React.FC = () => {
  ...
  const getAdminName = async (adminId: string): Promise<string> => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5002/api/admin/users/${adminId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  useEffect(() => {
    const fetchDeliveryMethods = async () => {
      try {
        const response = await fetch('http://localhost:5002/api/delivery/active');
  ...
  return (
    <div>
      ...
      {/* Фильтры и поиск */}
      <Card style={{ marginBottom: '24px', borderRadius: '12px' }} styles={{ body: { padding: 24 } }}>
