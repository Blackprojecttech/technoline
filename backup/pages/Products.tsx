...
const Products: React.FC = () => {
  ...
  const handleBulkActionConfirm = async (action: string, data: any) => {
    ...
    if (action === "delete") url = "/api/admin/products/bulk-delete";
    if (action === "restore") url = "/api/admin/products/bulk-restore";
    if (action === "hard-delete") url = "/api/admin/products/bulk-hard-delete";
    if (["category", "price", "stockQuantity", "costPrice", "isActive", "characteristic"].includes(action)) {
      url = "/api/admin/products/bulk-update";
      body.update = data;
    }
    if (!url) return;
