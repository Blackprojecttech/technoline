  ...
  try {
    ...
    try {
      ...
      const cashExpense = new Payment({
        ...
        inCashRegister: 'yes', // Наличные расходы в кассе по умолчанию
      ...
      console.log('💰 Создана запись о списании из кассы:', {
        ...
        debtId: debt.id,
        inCashRegister: cashExpense.inCashRegister,
        type: cashExpense.type,
...
// Удалить все долги поставщикам (только для админов) - ДОЛЖНО БЫТЬ ПЕРЕД /:id
router.delete('/clear-all', auth, admin, async (req, res) => {
  try {
    console.log('🗑️ Запрос на удаление всех долгов поставщикам от пользователя:', (req as any).user?.role);
    ...
    console.log(`📊 Найдено долгов поставщикам для удаления: ${count}`);

    if (count === 0) {
      return res.json({ message: 'Долги поставщикам не найдены', deletedCount: 0 });
    }

    // Находим все долги с оплаченными суммами для возврата в кассу
    const debtsWithPayments = await Debt.find({ paidAmount: { $gt: 0 } });
    
    // Создаем возвратные платежи для каждого оплаченного долга
    if (debtsWithPayments.length > 0) {
      for (const debt of debtsWithPayments) {
        try {
          const refundPaymentId = `refund_debt_clear_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
          ...
          const refundPayment = new Payment({
            ...
            date: new Date(),
            description: `Возврат в кассу при очистке долга поставщику "${debt.supplierName}"`,
            paymentMethod: 'наличные',
            apiType: 'income',
            supplierId: debt.supplierId,
            notes: `Возврат ${debt.paidAmount.toLocaleString('ru-RU')} ₽ в кассу при очистке всех долгов`,
            inCashRegister: 'yes',
          ...
          console.log(`💰 Создан возврат в кассу на сумму: ${debt.paidAmount} ₽ для долга ${debt.id}`);
        } catch (error) {
          console.error(`⚠️ Ошибка при создании возврата для долга ${debt.id}:`, error);
        }
      }
    }

    // Удаляем связанные платежи (расходы по долгам)
    ...
    console.log(`✅ Удалено долгов поставщикам: ${result.deletedCount}, возвращено в кассу: ${debtsWithPayments.reduce((sum, debt) => sum + debt.paidAmount, 0)} ₽`);
    
    return res.json({ 
      message: 'Все долги поставщикам удалены',
    ...
    return res.status(500).json({ error: 'Ошибка при очистке долгов' });
  }
});
