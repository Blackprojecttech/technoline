        ...
        if (item.arrivalId) {
          ...
          const arrivalItem = arrival.items.find(arrItem => {
            if (item.serialNumber) {
              ...
              // Для услуг проверяем только isService
              if (item.isService || arrItem.isService) {
                return arrItem.productName === item.productName && 
                       arrItem.isService === item.isService;
              }
              // Для обычных товаров проверяем isAccessory
              return arrItem.productName === item.productName &&
          ...
          if (!arrivalItem) {
            ...
            // Для аксессуаров проверяем доступное количество
...
// Получить список серийных номеров, на которые выбиты чеки
router.get('/serial-numbers', auth, admin, async (req, res) => {
  try {
    // Получаем все чеки со статусом completed (не cancelled и не refunded)
    const receipts = await Receipt.find({ 
      status: 'completed',
      'items.serialNumber': { $exists: true, $ne: '' } // только чеки с серийными номерами
    });
    
    // Собираем все серийные номера из чеков
    const serialNumbers = receipts.reduce((acc: string[], receipt) => {
      if (receipt.items && Array.isArray(receipt.items)) {
        receipt.items.forEach(item => {
          if (item.serialNumber && typeof item.serialNumber === 'string' && item.serialNumber.trim()) {
            acc.push(item.serialNumber.trim());
          }
        });
      }
      return acc;
    ...
    // Возвращаем уникальный список серийных номеров
    const uniqueSerialNumbers = [...new Set(serialNumbers)];
    console.log(`Found ${uniqueSerialNumbers.length} unique serial numbers in completed receipts`);
    return res.json(uniqueSerialNumbers);
  } catch (error) {
    console.error('Error fetching serial numbers from receipts:', error);
    return res.status(500).json({ error: 'Ошибка при получении серийных номеров из чеков' });
  }
});
