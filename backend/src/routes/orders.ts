...
router.post('/', async (req, res) => {
  try {
    ...
    if (!userId && (email || phone)) {
      ...
      if (!user) {
        // Создаем нового пользователя без пароля (гостевой заказ)
        user = new User({
          email: email,
          phone: phone,
        ...
        await user.save();
        console.log('✅ Создан новый пользователь (гостевой заказ):', user._id);
      }

      // Автоматически связываем с реферальной системой (если есть реферальный код)
      const { linkOrderToReferralSystem } = await import('../utils/referralHelper');
      await linkOrderToReferralSystem(req, user._id as mongoose.Types.ObjectId);
    ...
    // Отправляем push-уведомление о новом заказе
    if (req.body.userId) {
      try {
        ...
        console.log('✅ Отправлено push-уведомление о новом заказе');
      } catch (pushError) {
        console.error('❌ Ошибка отправки push-уведомления о заказе:', pushError);
      }
    }
...
router.put('/:id', auth, adminOnly, async (req: AuthRequest, res) => {
  try {
    ...
    // Обработка реферальной комиссии при смене статуса
    if (req.body.status && req.body.status !== order.status) {
      try {
        // Начисляем комиссию при смене статуса на "delivered"
        if (req.body.status === 'delivered' && order.status !== 'delivered') {
          console.log('💰 Обрабатываем реферальную комиссию для заказа:', updatedOrder.orderNumber);
          const { processReferralCommission } = await import('../controllers/referralController');
          await processReferralCommission(String(updatedOrder._id));
        }
        
        // Отменяем комиссию при смене статуса с "delivered" на другой
        if (order.status === 'delivered' && req.body.status !== 'delivered') {
          console.log('💸 Отменяем реферальную комиссию для заказа:', updatedOrder.orderNumber);
          const { cancelReferralCommission } = await import('../controllers/referralController');
          await cancelReferralCommission(String(updatedOrder._id));
        }
      } catch (error) {
        console.error('❌ Ошибка при обработке реферальной комиссии:', error);
      }
    }
    ...
    // Отправляем push-уведомление об изменении статуса заказа
    if (changedFields.status && updatedOrder.userId) {
      try {
        ...
        console.log('✅ Отправлено push-уведомление об изменении статуса заказа');
      } catch (pushError) {
        console.error('❌ Ошибка отправки push-уведомления о статусе:', pushError);
      }
    }
    ...
    if (
      ...
      // Получаем заказ с полной информацией о продуктах
      const populatedOrder = await Order.findById(updatedOrder._id)
        .populate('items.productId', 'name slug _id');

      if (!populatedOrder) {
        throw new Error('Order not found after population');
      }

      // Для каждого товара заказа создать уведомление
      for (const item of populatedOrder.items) {
        // Берём slug из популированного productId
        let link = (item.productId && typeof item.productId === 'object' && 'slug' in item.productId && item.productId.slug)
          ...
          : undefined;
        // Получаем ID продукта для проверки дублей
        const productId = (item.productId && typeof item.productId === 'object' && '_id' in item.productId) 
          ? item.productId._id 
          : item.productId;
        ...
        if (!existingNotification) {
          // Проверяем, что у нас есть productId и это объект с нужными полями
          if (item.productId && typeof item.productId === 'object' && 'slug' in item.productId && item.productId.slug) {
            console.log('🔗 Создаем уведомление с данными продукта:', {
              name: item.name,
            ...
            console.log('❌ Не удалось создать уведомление, данные продукта:', item.productId);
          }
        }
      }
    }
...
router.put('/:id', auth, adminOnly, async (req: AuthRequest, res) => {
  try {
    ...
    // Обработка реферальной комиссии при смене статуса в полном обновлении
    if (req.body.status && req.body.status !== oldValues.status) {
      try {
        // Начисляем комиссию при смене статуса на "delivered"
        if (req.body.status === 'delivered' && oldValues.status !== 'delivered') {
          console.log('💰 Обрабатываем реферальную комиссию для заказа (полное обновление):', updatedOrder.orderNumber);
          const { processReferralCommission } = await import('../controllers/referralController');
          await processReferralCommission(String(updatedOrder._id));
        }
        
        // Отменяем комиссию при смене статуса с "delivered" на другой
        if (oldValues.status === 'delivered' && req.body.status !== 'delivered') {
          console.log('💸 Отменяем реферальную комиссию для заказа (полное обновление):', updatedOrder.orderNumber);
          const { cancelReferralCommission } = await import('../controllers/referralController');
          await cancelReferralCommission(String(updatedOrder._id));
        }
      } catch (error) {
        console.error('❌ Ошибка при обработке реферальной комиссии (полное обновление):', error);
      }
    }
