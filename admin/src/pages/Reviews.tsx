...
const ReviewsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/reviews`, {
      headers: {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/reviews/${id}`, {
            method: 'DELETE',
            headers: {
              ...(token ? { 'Authorization': `Bearer ${token}` } : {})
            }
      ...
      onOk: async () => {
        try {
          ...
          await Promise.all(selectedRowKeys.map(async (id) => {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/reviews/${id}`, {
              method: 'DELETE',
              headers: {
                ...(token ? { 'Authorization': `Bearer ${token}` } : {})
              }
  ...
  const handleEditSave = async (values: any) => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/reviews/${editingReview._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleAnswerSave = async () => {
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('admin_token') : null;
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/reviews/${answeringReview._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
  ...
  const handleApprove = async (id: string, checked: boolean) => {
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('admin_token') : null;
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/reviews/${id}/approve`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
