...
import { Table, Button, Space, Tag, Modal, message, Form, Input, InputNumber, Switch, Card, Typography, Spin, Select, TimePicker } from 'antd'
...
// Кастомный компонент TimePicker с защитой от невалидных значений
const SafeTimePicker: React.FC<any> = ({ value, onChange, ...props }) => {
  // Преобразуем значение в безопасный формат
...
const DeliveryMethods: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/admin/delivery-zones`, {
  ...
  const updateDeliveryMutation = useMutation({
    mutationFn: async ({ id, values }: { id: string, values: any }) => {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/delivery/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
  ...
  const handleEdit = (record: DeliveryMethod) => {
    ...
    form.setFieldsValue({
      ...
      orderTimeFrom: record.orderTimeFrom,
      orderTimeTo: record.orderTimeTo,
      deliveryTodayTimeFrom: record.deliveryTodayTimeFrom,
      deliveryTodayTimeTo: record.deliveryTodayTimeTo,
      deliveryTomorrowTimeFrom: record.deliveryTomorrowTimeFrom,
      deliveryTomorrowTimeTo: record.deliveryTomorrowTimeTo,
      orderTimeForToday: record.orderTimeForToday,
      orderTimeForTomorrow: record.orderTimeForTomorrow,
  ...
  // Функция для преобразования объекта Day.js в строку времени
  const convertDayjsToString = (dayjsObj: any): string | null => {
    if (!dayjsObj) return null;
    
    console.log('🔄 Converting dayjs to string:', dayjsObj, 'type:', typeof dayjsObj);
    
    try {
      if (dayjs.isDayjs(dayjsObj)) {
        const result = dayjsObj.format('HH:mm');
        console.log('✅ Converted to string:', result);
        return result;
      }
      
      // Если это строка в формате HH:mm, возвращаем как есть
      if (typeof dayjsObj === 'string' && /^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(dayjsObj)) {
        console.log('✅ Already in HH:mm format:', dayjsObj);
        return dayjsObj;
      }
      
      console.log('❌ Not a valid dayjs object or time string');
      return null;
    } catch (error) {
      console.error('❌ Error converting dayjs to string:', error);
      return null;
    }
  };
  ...
  const onFinish = (values: any) => {
    ...
    // Преобразуем объекты Day.js в строки времени
    ...
    timeFields.forEach(field => {
      if (processedValues[field] !== undefined) {
        console.log(`🕒 Processing field ${field}:`, processedValues[field]);
        processedValues[field] = convertDayjsToString(processedValues[field]);
      }
    });
    ...
    <div style={{ padding: '24px' }}>
      ...
      >
        ...
        >
          ...
          <Form.Item
            name="deliveryTodayTimeFrom"
            label="Время доставки сегодня с"
          >
            <SafeTimePicker
              format="HH:mm"
              placeholder="Выберите время"
          ...
          <Form.Item
            name="deliveryTodayTimeTo"
            label="Время доставки сегодня до"
          >
            <SafeTimePicker
              format="HH:mm"
              placeholder="Выберите время"
          ...
          <Form.Item
            name="orderTimeForToday"
            label="Заказ до (для доставки сегодня)"
          >
            <SafeTimePicker
              format="HH:mm"
              placeholder="Выберите время"
          ...
          <Form.Item
            name="deliveryTomorrowTimeFrom"
            label="Время доставки завтра с"
          >
            <SafeTimePicker
              format="HH:mm"
              placeholder="Выберите время"
          ...
          <Form.Item
            name="deliveryTomorrowTimeTo"
            label="Время доставки завтра до"
          >
            <SafeTimePicker
              format="HH:mm"
              placeholder="Выберите время"
          ...
          <Form.Item
            name="orderTimeForTomorrow"
            label="Заказ до (для доставки завтра)"
          >
            <SafeTimePicker
              format="HH:mm"
              placeholder="Выберите время"
