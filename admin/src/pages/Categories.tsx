...
import { Tree, Button, Modal, Form, Input, message, Spin, Space, Tag, Switch, TreeSelect, Card, Typography, Popconfirm, Tooltip } from 'antd'
...
export default function Categories() {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const url = editingCategory
        ? `${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/categories/${editingCategory._id}`
        : `${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/categories`;
  ...
  const onDrop = async (info: any) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/categories/${dragKey}/move`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
  ...
  const handleDelete = async (categoryId: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5002/api'}/categories/${categoryId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
        }
