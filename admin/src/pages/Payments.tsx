...
import { paymentsApi, receiptsApi, incassateReceipts, debtsApi, simpleDebtsApi } from '../utils/baseApi';
...
import { arrivalsApi } from '../utils/baseApi';
// simpleDebtsApi теперь импортируется из baseApi
...
const PaymentsTab: React.FC<{
  ...
  // Состояния для оплаты долгов (старый код - удален)
  ...
  // Состояния для оплаты долгов
  const [payDebtModalVisible, setPayDebtModalVisible] = useState(false);
  const [selectedDebt, setSelectedDebt] = useState<PaymentItem | null>(null);
  const [payDebtForm] = Form.useForm();
  ...
  // Обработчик оплаты долга
  const handlePayDebt = (debt: PaymentItem) => {
    setSelectedDebt(debt);
    setPayDebtModalVisible(true);
    payDebtForm.resetFields();
  };

  // Подтверждение оплаты долга
  const handlePayDebtConfirm = async (values: any) => {
    if (!selectedDebt) return;
    
    try {
      setLoadingPayments(true);
      
      // Определяем тип долга и ID
      let debtId = '';
      let isSimpleDebt = false;
      
      if (selectedDebt.id.startsWith('simple_debt_')) {
        debtId = selectedDebt.id.replace('simple_debt_', '');
        isSimpleDebt = true;
      } else if (selectedDebt.id.startsWith('debt_')) {
        debtId = selectedDebt.id.replace('debt_', '');
        isSimpleDebt = false;
      }
      ...
      let response;
      if (isSimpleDebt) {
        response = await simpleDebtsApi.pay(debtId, { amount: values.amount });
      } else {
        response = await debtsApi.pay(debtId, { amount: values.amount });
      }
      
      message.success(`Долг оплачен на сумму ${values.amount.toLocaleString('ru-RU')} ₽`);
      ...
      message.error('Ошибка при оплате долга');
    } finally {
      setLoadingPayments(false);
    }
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          // Не вызываем автоматическое обновление после очистки,
          // так как это приводит к повторному созданию платежей из чеков
          console.log('✅ Очистка завершена. Автоматическое обновление отключено.');
  ...
  const getDebtStatusTag = (status: string) => {
    switch (status) {
      case 'active':
        return <Tag color="blue">Активный</Tag>;
      case 'partially_paid':
        return <Tag color="orange">Частично оплачен</Tag>;
      case 'paid':
        return <Tag color="green">Оплачен</Tag>;
      case 'overdue':
        return <Tag color="red">Просрочен</Tag>;
      default:
        return null;
    }
  };
  ...
  const columns: ColumnsType<PaymentItem> = [
    ...
    },
    {
      ...
      key: 'debtStatus',
      render: (_, record) => {
        if (record.paymentMethod === 'debt') {
          return getDebtStatusTag(record.status || 'active');
        }
        return null;
      }
    },
    ...
    },
    {
      ...
      width: 120,
      render: (_, record) => {
        const isDebt = record.id && (record.id.startsWith('debt_') || record.id.startsWith('simple_debt_'));
        const isReceipt = record.id && record.id.startsWith('receipt_');
        const isIncassation = record.category === 'Инкассация';
        
        return (
          <Space size="small">
            {/* Для долгов показываем кнопку оплаты */}
            {isDebt && (
              <Tooltip title="Оплатить долг">
                <Button
                  type="primary"
                  size="small"
                  icon={<DollarOutlined />}
  ...
  const filteredPayments = payments
    .filter(payment => {
      ...
      if (incassationFilter === 'active') {
        ...
        matchesIncassation = payment.status === 'incassated' || payment.category === 'Инкассация';
      } else if (incassationFilter === 'debts') {
        matchesIncassation = payment.category === 'Оплата долга поставщику' || 
                           payment.category === 'Оплата долга' ||
                           payment.category === 'Долг поставщику' ||
                           payment.category === 'Простой долг' ||
                           payment.id?.startsWith('supplier_debt_') ||
                           payment.id?.startsWith('simple_debt_');
      }
  ...
  return (
    <div>
      ...
      {/* Модальное окно оплаты долга */}
      <Modal
        title="Оплата долга"
        open={payDebtModalVisible}
        onOk={() => payDebtForm.submit()}
        onCancel={() => {
          setPayDebtModalVisible(false);
          setSelectedDebt(null);
          payDebtForm.resetFields();
      ...
      >
        <Form
          form={payDebtForm}
          layout="vertical"
          onFinish={handlePayDebtConfirm}
        >
          {selectedDebt && (
            <div style={{ marginBottom: 16, padding: 12, background: '#f5f5f5', borderRadius: 6 }}>
              <div style={{ fontWeight: 500, marginBottom: 4 }}>
                {selectedDebt.id.startsWith('simple_debt_') ? 'Простой долг' : 'Долг поставщику'}
              </div>
              <div style={{ fontSize: 12, color: '#666' }}>
                {selectedDebt.description}
              </div>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#ff4d4f', marginTop: 4 }}>
                Сумма долга: {Math.abs(selectedDebt.amount).toLocaleString('ru-RU')} ₽
              </div>
            </div>
          )}
          ...
          <Form.Item
            ...
            rules={[
              ...
              { 
                type: 'number',
                min: 1,
                max: selectedDebt ? Math.abs(selectedDebt.amount) : undefined,
                message: `Сумма должна быть от 1 до ${selectedDebt ? Math.abs(selectedDebt.amount).toLocaleString('ru-RU') : 0} ₽`
              }
            ]}
          >
            <InputNumber
          ...
          <div style={{ padding: 8, background: '#fffbe6', border: '1px solid #ffe58f', borderRadius: 4 }}>
            <InfoCircleOutlined style={{ color: '#faad14', marginRight: 8 }} />
            Будет создана запись о расходе и обновлен статус долга
          </div>
        </Form>
      </Modal>
    </div>
...
const ExpensesTab: React.FC = () => {
  ...
  const filteredExpenses = expenses.filter(expense => {
    // Сначала проверяем фильтр по категории
    if (selectedCategory && expense.category !== selectedCategory) {
      return false;
    }
    // Затем проверяем поисковый запрос
    return expense.description.toLowerCase().includes(searchText.toLowerCase()) ||
    ...
    <div>
      ...
      <Card style={{ marginBottom: '24px', borderRadius: '12px' }}>
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          />
          <Button
            type={selectedCategory === 'Аванс' ? 'primary' : 'default'}
            onClick={() => {
              if (selectedCategory === 'Аванс') {
                setSelectedCategory(null);
              } else {
                setSelectedCategory('Аванс');
              }
            }}
            icon={<WalletOutlined />}
          >
...
const PurchasesTab: React.FC<{
  ...
  // Слушаем события удаления прихода и возврата
  useEffect(() => {
    const handleArrivalEvents = (e: CustomEvent) => {
      console.log('🔔 PurchasesTab: Получено событие:', e.type, e.detail);
      
      // Определяем ID прихода в зависимости от типа события
      let deletedArrivalId;
      if (e.type === 'arrivalDeleted') {
        deletedArrivalId = e.detail?.arrivalId;
      } else if (e.type === 'refundCreated') {
        deletedArrivalId = e.detail?.arrivalId || e.detail?.payment?.arrivalId;
      }

      if (deletedArrivalId) {
        console.log('🗑️ Приход удален, ищем связанную покупку с arrivalId:', deletedArrivalId);
        
        // Загружаем актуальные покупки из localStorage
        const storedPurchases = JSON.parse(localStorage.getItem('admin_purchases') || '[]');
        console.log('🔍 Текущие покупки из localStorage:', storedPurchases.map((p: any) => ({ id: p.id, productName: p.productName, arrivalId: p.arrivalId })));
        
        // Находим и удаляем покупку, связанную с этим приходом
        const updatedPurchases = storedPurchases.filter((p: any) => {
          // Проверяем, что arrivalId не совпадает с удаленным приходом
          // Важно: сравниваем как строки, так как MongoDB ID может быть в разных форматах
          return p.arrivalId?.toString() !== deletedArrivalId.toString();
        });
        
        console.log('📊 Результат фильтрации:', {
          before: storedPurchases.length,
          after: updatedPurchases.length,
          deletedArrivalId
        });
        
        if (updatedPurchases.length !== storedPurchases.length) {
        ...
        console.log('❌ Событие не содержит arrivalId');
      }
    };

    window.addEventListener('arrivalDeleted', handleArrivalEvents as EventListener);
    window.addEventListener('refundCreated', handleArrivalEvents as EventListener);
    
    return () => {
      window.removeEventListener('arrivalDeleted', handleArrivalEvents as EventListener);
      window.removeEventListener('refundCreated', handleArrivalEvents as EventListener);
    };
  }, []);
  ...
  const handleModalOk = async () => {
    try {
      ...
      // Важно: newPurchase теперь содержит arrivalId после createProductAndArrival
      const updatedPurchases = [newPurchase, ...purchases];
      setPurchases(updatedPurchases);
      
      // Сохраняем в localStorage с обновленным arrivalId
      localStorage.setItem('admin_purchases', JSON.stringify(updatedPurchases));
    ...
    try {
      ...
      if (!categoryId) {
        ...
        const productData = {
          ...
          sku: values.article, // Передаем артикул как SKU товара
          price: values.price,
          costPrice: values.costPrice,
          ...
          isActive: values.isHidden !== undefined ? !values.isHidden : false, // Инвертируем: если isHidden = true, то isActive = false
          ...
          isFromDatabase: true // Помечаем, что товар из базы
        };
  ...
  const handleDeletePurchase = async (purchase: PurchaseItem) => {
    Modal.confirm({
      title: 'Удалить покупку?',
      content: (
        <div>
          <p>Покупка "{purchase.productName}" будет удалена.</p>
          <p>Связанный приход также будет удален автоматически.</p>
        </div>
      ),
      okText: 'Удалить',
      okType: 'danger',
      ...
      onOk: async () => {
        try {
          // Если есть связанный приход, удаляем его
          if (purchase.arrivalId) {
            await arrivalsApi.delete(purchase.arrivalId);
          }

          // Удаляем покупку из локального состояния
          const updatedPurchases = purchases.filter(p => p.id !== purchase.id);
          setPurchases(updatedPurchases);
          localStorage.setItem('admin_purchases', JSON.stringify(updatedPurchases));
          
          message.success('Покупка и связанный приход удалены');
        } catch (error: any) {
          // Если ошибка содержит сообщение от сервера, показываем его
          if (error.data?.error) {
            Modal.error({
            ...
            message.error('Ошибка при удалении покупки');
          }
        }
      }
    });
...
const Payments: React.FC = () => {
  ...
  const refreshPayments = async (showMessage = true) => {
    try {
      ...
      const debtsResponse = await debtsApi.getAll();
      const simpleDebtsResponse = await simpleDebtsApi.getAll(); // Добавляем загрузку простых долгов

      const basePayments = response.data || [];
      const receiptsData = receiptsResponse.data || [];
      const debtsData = debtsResponse.data || [];
      const simpleDebtsData = simpleDebtsResponse.data || []; // Получаем данные о простых долгах

      // Фильтруем валидные платежи
      const validBasePayments = basePayments.filter((payment: any) => payment && payment.id);

      // Интегрируем чеки
      const integratedPayments = integrateReceiptsIntoPayments(validBasePayments, receiptsData);

      // Интегрируем долги поставщикам и простые долги
      const allPayments = integratedPayments.concat(
        // Долги поставщикам
        debtsData.map((debt: any) => ({
          id: `supplier_debt_${debt.id}`,
          type: 'transfer',
          amount: -debt.remainingAmount,
          description: `Долг поставщику: ${debt.supplierName} - ${debt.notes || 'Без описания'}`,
          date: debt.date,
          supplier: debt.supplierName,
          inCashRegister: false,
          category: 'Долг поставщику',
          paymentMethod: 'debt',
          apiType: 'expense',
          status: debt.status,
          notes: `Общая сумма: ${debt.amount}₽\nОплачено: ${debt.paidAmount}₽\nОсталось: ${debt.remainingAmount}₽\n\nТовары:\n${debt.items.map((item: any) => 
            `• ${item.productName} - ${item.quantity} шт. по ${item.costPrice}₽${item.serialNumbers?.length ? ` (S/N: ${item.serialNumbers.join(', ')})` : ''}`
          ).join('\n')}`,
          createdAt: debt.createdAt
        })),
        // Простые долги
        simpleDebtsData
          .filter((debt: any) => debt.status !== 'paid') // Показываем только неоплаченные
          .map((debt: any) => ({
          id: `simple_debt_${debt.id}`,
          type: 'transfer',
          amount: -debt.remainingAmount,
          description: `Простой долг: ${debt.title}`,
          date: debt.date,
          inCashRegister: false,
          category: 'Простой долг',
          paymentMethod: 'simple_debt',
          apiType: 'expense',
          status: debt.status,
          notes: `Общая сумма: ${debt.amount}₽\nОплачено: ${debt.paidAmount}₽\nОсталось: ${debt.remainingAmount}₽\n\nПримечание: ${debt.notes || 'Нет'}`,
          createdAt: debt.createdAt
        }))
      );
      ...
      if (showMessage) {
        message.success('Платежи обновлены');
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const handlePaymentUpdated = (e: CustomEvent) => {
      ...
      // Обрабатываем оплату долга
      if (e.detail?.type === 'debtPayment') {
        console.log('💰 Получено событие оплаты долга:', e.detail);
        const { amount, supplier, debtId } = e.detail;
        
        // Создаем запись об оплате долга
        const payment: PaymentItem = {
          id: `debt_payment_${Date.now()}`,
          type: 'cash',
          amount: -Math.abs(amount),
          description: `Оплата долга поставщику: ${supplier || 'Без названия'}`,
        ...
        // Обновляем список платежей через 2 секунды для гарантии обновления из БД
        setTimeout(() => {
          console.log('💰 Обновляем платежи через 2 секунды после оплаты долга...');
